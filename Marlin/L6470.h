...
#define dSPIN_BUSYN_Z     15   // Wire this to the BSYN line
#define dSPIN_FLAG_Z      17   // Wire this to the FLAG line

#define MOTOR_COUNT      4

#define MOTOR_X          X_AXIS
#define MOTOR_Y          Y_AXIS
#define MOTOR_Z          Z_AXIS
#define MOTOR_E          E_AXIS
...
//#define WRITE_L6470(motor, pin, direction) dSPIN_Step_Clock(motor, direction);
//#define WRITE_L6470_STEP(motor, pin, state) WRITE(pin, state);
#define WRITE_L6470_STEP(motor, pin, state);  // Do nothing NOOP as stepping is not used by the L6470's
#define WRITE_L6470(motor, pin, direction);  // Currently is a NOOP, but could be used to record the direction which is used later by the move command.
#endif
