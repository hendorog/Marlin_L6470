...
static volatile unsigned char	L6470_x_dir,
			L6470_y_dir,
			L6470_z_dir,
                        L6470_e_dir;
...
void st_init()
{
  ...
  //endstops and pullups
    ...
    SET_OUTPUT(Y_MIN_PIN); //Added  as doing detection in L6470. Set as an output now so we can drive the existing code.
...
boolean L6470_positioning_mode;
    ...
    if(L6470_status[device] != L6470_oldstatus[device]) {  // Has status changed for this device? If nothing has changed then do nothing.
      ...
      if(device == MOTOR_X) {
        if(L6470_status[device] & dSPIN_STATUS_SW_F) {  // Make the existing endstop code mirror the switch bit in the L6470 X axis
          WRITE(X_MIN_PIN, !X_ENDSTOPS_INVERTING); // Drive the existing code low, this PIN has been set to be an output now.
        } else {
      ...
      if(device == MOTOR_Y) {
        if(L6470_status[device] & dSPIN_STATUS_SW_F) {  // Make the existing endstop code mirror the switch bit in the L6470 Y axis
          WRITE(Y_MIN_PIN, !Y_ENDSTOPS_INVERTING); // Drive the existing code low, this PIN has been set to be an output now.
        } else {
      ...
      if(device == MOTOR_Z) {
        if(L6470_status[device] & dSPIN_STATUS_SW_F) {  // Make the existing endstop code mirror the switch bit in the L6470 Z axis
          WRITE(Z_MIN_PIN, !Z_ENDSTOPS_INVERTING); // Drive the existing code low, this PIN has been set to be an output now.
        } else {
      ...
      if(L6470_status[device] & dSPIN_STATUS_SW_EVN) { // A Switch event has occurred. Device position will be reset automatically.
        ...
        if(L6470_status[device] & dSPIN_STATUS_SW_F) { // Endstop is on
          ...
          if(L6470_status[device] & dSPIN_STATUS_DIR) { //  and we are trying to Move in reverse deeper into the endstop (note that this currently assumes axis are not reversed!!)
            ...
            SERIAL_ECHO_START;
            SERIAL_ECHOPAIR("Device no: ", (unsigned long)device);
            SERIAL_ECHOLN("Reverse Movement stopped because dSPIN_STATUS_SW_F is active");
           }
        }
      }
      
      // *** Busy Status updates *** //
      if(L6470_positioning_mode) { // are we going to a position or running forever at a given speed.
		  if(device == MOTOR_X) {
			boolean x_busy_temp = dSPIN_IsBusy_Status(L6470_status[device]);
			x_busy = x_busy_temp;
			L6470_run_mode_error[MOTOR_X] = 0L;
		  ...
		  if(device == MOTOR_Y) {
			boolean y_busy_temp = dSPIN_IsBusy_Status(L6470_status[device]);
			y_busy = y_busy_temp;
			#ifdef STEPPER_DEBUG_BUSY
		  ...
		  if(device == MOTOR_Z) {
			boolean z_busy_temp = dSPIN_IsBusy_Status(L6470_status[device]);
			z_busy = z_busy_temp;
			#ifdef STEPPER_DEBUG_BUSY
		  ...
		  if(device == MOTOR_E) {
			boolean e_busy_temp = dSPIN_IsBusy_Status(L6470_status[device]);
			e_busy = e_busy_temp;
			#ifdef STEPPER_DEBUG_BUSY
			  SERIAL_ECHOPAIR("E Busy: ", (unsigned long)e_busy);
			#endif
		  }
      	} else { // Run mode not motion control
  			unsigned long steps = current_pos[device] - start_pos[device];
      		if(device == MOTOR_X) {
      			if(((abs(steps) - current_block->steps_x) < X_MICROSTEPS) || (steps > current_block->steps_x)) { // <-- near enough to call it finished
      	  			L6470_run_mode_error[device] += steps;
      				x_busy = false;
      			}
      		}
      		if(device == MOTOR_Y) {
      			if(((abs(steps) - current_block->steps_y) < Y_MICROSTEPS) || (steps > current_block->steps_y)) { // <-- near enough to call it finished
      				L6470_run_mode_error[device] += steps;
      				y_busy = false;
      			}
      		}
      		if(device == MOTOR_Z) {
      			if(((abs(steps) - current_block->steps_z) < Z_MICROSTEPS) || (steps > current_block->steps_z)) { // <-- near enough to call it finished
      				L6470_run_mode_error[device] += steps;
      				z_busy = false;
      			}
      		}
      		if(device == MOTOR_E) {
      			if(((abs(steps) - current_block->steps_e) < E_MICROSTEPS) || (steps > current_block->steps_e)) { // <-- near enough to call it finished
      				L6470_run_mode_error[device] += steps;
      				e_busy = false;
      			}
      		}
      ...
      if(!L6470_status[device] & dSPIN_STATUS_TH_SD ) {
        ...
        SERIAL_ECHO_START;
        SERIAL_ECHOPAIR("Device no: ", (unsigned long)device);
        SERIAL_ECHOPAIR("STATUS_TH_SD: ", ++thermal_shutdown_counter[device]); SERIAL_ECHOLN("");
      }
      ...
      if(!(L6470_status[device] & dSPIN_STATUS_TH_WRN )) { 
        SERIAL_ECHOPAIR("STATUS_TH_WRN: ", ++thermal_warning_counter[device]); SERIAL_ECHOLN("");
      }
      
      
      if(!(L6470_status[device] & dSPIN_STATUS_STEP_LOSS_A ) || !(L6470_status[device] & dSPIN_STATUS_STEP_LOSS_B )) { 
        SERIAL_ECHOPAIR("STEP_LOST ", ++step_loss_counter[device]); SERIAL_ECHOLN("");
      }
      
      
      SERIAL_ECHOPAIR("Device no: ", (unsigned long)device);
      SERIAL_ECHOLN("");
      if(L6470_status[device] & dSPIN_STATUS_HIZ ) SERIAL_ECHOLN("STATUS_HIZ ");
      ...
      if(L6470_status[device] & dSPIN_STATUS_NOTPERF_CMD ) SERIAL_ECHOLN("STATUS_NOTPERF_CMD ");
      if(L6470_status[device] & dSPIN_STATUS_WRONG_CMD ) SERIAL_ECHOLN("STATUS_WRONG_CMD ");
      ...
      if(L6470_status[device] & dSPIN_STATUS_SCK_MOD ) SERIAL_ECHOLN("STATUS_SCK_MOD ");
    }
  }
}
  ...
  if(steppers_enabled) {      
    ...
    if (current_block != NULL) {
      ...
      if(L6470_load_move) {     
           ...
           SERIAL_ECHOLN("");       
           SERIAL_ECHOPAIR(" Steps X: ", (unsigned long) current_block->steps_x);
           SERIAL_ECHOLN("");       
           ...
           // Get the current step position for all axis
           dSPIN_GetParam_All(dSPIN_ABS_POS, start_pos);
          ...
          for(int i = 0; i < MOTOR_COUNT; i++) {
            if(i == MOTOR_X) {
              ...
              min_speed[i] = MinSpdCalc(current_block->entry_speed * axis_steps_per_unit[MOTOR_X]);
            ...
            if(i == MOTOR_Y) {
              ...
              min_speed[i] = MinSpdCalc(current_block->entry_speed * axis_steps_per_unit[MOTOR_Y]);
            ...
            if(i == MOTOR_Z) {
              ...
              min_speed[i] = MinSpdCalc(current_block->entry_speed * axis_steps_per_unit[MOTOR_Z]);
            ...
            if(i == MOTOR_E) {
              ...
              min_speed[i] = MinSpdCalc(current_block->entry_speed * axis_steps_per_unit[MOTOR_E]);
              max_speed[i] = MaxSpdCalc(current_block->rate_e);
              accel[i] = AccCalc(current_block->accel_e * 1.1);      // <-- Accel slightly faster to bias the extrustion towards the start of the movement. This helps avoid the extruder running too long which seems to occur at slower feed rates.
          ...
          // Therefore try to avoid stopping, this method is less accurate, but means the the moves don't stop
          if(current_block->millimeters < SHORT_MOVE_MM) {
        	  L6470_run_mode = true;
        	  dSPIN_Run_All(dir, max_speed);
          } else {
      ...
      if(!L6470_complete) { // Move not finished, so check to see if the motors are still running and update all status from the motors
        ...
        if(!x_busy && !y_busy && !z_busy) {
          if(e_busy && ((current_block->steps_x > 0) || (current_block->steps_y > 0) || (current_block->steps_z > 0))) {  
            dSPIN_HardStop(MOTOR_E);
            #ifdef STEPPER_DEBUG
              SERIAL_ECHO_START;            
              SERIAL_ECHOLN("L6470 Extruder still busy after movement complete.");
            #endif
          }
        }
        ...
        if(x_busy || y_busy || z_busy || e_busy) {
          ...
          #ifdef STEPPER_DEBUG
            SERIAL_ECHO_START;
            if(L6470_complete)        SERIAL_ECHOLN("L6470 movement complete flag set")      else        SERIAL_ECHOLN("L6470 movement complete flag not set");
          #endif      
        } // if not busy
      } // if !complete
